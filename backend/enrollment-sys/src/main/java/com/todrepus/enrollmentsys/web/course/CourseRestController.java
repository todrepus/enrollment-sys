package com.todrepus.enrollmentsys.web.course;
...
import com.todrepus.enrollmentsys.web.course.dto.AddCourseDTO;
import com.todrepus.enrollmentsys.web.course.dto.CourseResponseDTO;
import com.todrepus.enrollmentsys.web.course.dto.UpdateCourseDTO;
...
public class CourseRestController {
    ...
    public RestResponseDTO<List<CourseResponseDTO>> getCourseList(@RequestParam(required = false) Integer page){
        ...
        Page<Course> pageCourse = courseService.findCoursesOnPage(page, AdminPageConst.ELEMENT_NUM);
        List<Course> courses = pageCourse.getContent();
        int maxPage = pageCourse.getTotalPages();
        List<CourseResponseDTO> coursesOnPage = courses.stream()
    ...
    public RestResponseDTO<CourseResponseDTO> addCourse(@Validated @RequestBody AddCourseDTO addCourseDTO,
        ...
        List<CourseSchedule> courseScheduleList = addCourseDTO.getScheduleList().stream().map(
                courseScheduleDTO ->
                    CourseSchedule.builder()
                            .courseHourEnd(courseScheduleDTO.getEndHour())
                            .courseMinEnd(courseScheduleDTO.getEndMin())
                            .courseMinStart(courseScheduleDTO.getStartMin())
                            .courseHourStart(courseScheduleDTO.getStartHour())
                            .courseDay(courseScheduleDTO.getDay())
                            .build()
        ).toList();
    ...
    public RestResponseDTO<CourseResponseDTO> updateCourse(@PathVariable Long courseId, @Validated @RequestBody UpdateCourseDTO updateCourseDTO,
        ...
        // 강의 스케줄 리스트
        if (updateCourseDTO.getScheduleList() != null) {
            List<CourseSchedule> courseScheduleList = updateCourseDTO.getScheduleList().stream().map(
                    courseScheduleDTO -> {
