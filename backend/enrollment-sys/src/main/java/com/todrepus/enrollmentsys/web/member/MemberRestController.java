package com.todrepus.enrollmentsys.web.member;
...
import com.todrepus.enrollmentsys.web.member.dto.*;
...
public class MemberRestController {
    ...
    public RestResponseDTO<List<MemberResponseDTO>> getMembers(@RequestParam(required = false) Integer page) {
        ...
        Page<Member> pageMember = memberService.findMembersOnPage(page, AdminPageConst.ELEMENT_NUM);
        List<Member> members = pageMember.getContent();
        int maxPage = pageMember.getTotalPages();
        List<MemberResponseDTO> membersOnPage = members.stream()
    ...
    @PostMapping("/students/{id}/enrolls/add")
    public RestResponseDTO<StudentResponseDTO> enrollCourse(@PathVariable Long id, @RequestParam Long courseId, HttpServletResponse httpServletResponse){
        Student student = memberService.findStudentById(id);
        Course course = courseService.findCourse(courseId);
    ...
    @PostMapping("/students/{id}/enrolls/cancel")
    public RestResponseDTO<StudentResponseDTO> cancelEnroll(@PathVariable Long id, @RequestParam Long enrollId, HttpServletResponse httpServletResponse){
        Student student = memberService.findStudentById(id);
        CourseEnroll courseEnroll = courseEnrollService.findCourseEnrollById(enrollId);
        boolean success = memberService.cancelCourseEnroll(student, courseEnroll);

        if (!success){
            httpServletResponse.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            return RestResponseDTO.getInternalErrorResponse("강의취소 실패");
        }

        RestResponseDTO<StudentResponseDTO> responseDTO =
                RestResponseDTO.getSuccessResponse("강의취소 성공");
        responseDTO.setData(new StudentResponseDTO(student));
        return responseDTO;
    }
}
